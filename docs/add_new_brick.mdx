...
Deshalb solltest du *ALLE* Tests deines Features innerhalb deines Bricks in einen Ordner ```test``` packen (den Ordner kannst du auch anders nennen, mehr dazu [unten][new_creation_option_link]) und innerhalb dieses Ordners die Brickstruktur nachbauen.
...
Zum Bundlen kannst du ganz entspannt das ```./bundle_brick.sh``` Skript nutzen. Übergib dafür als Argument den Namen deines Bricks. Beispiel: dein Brick heißt ```awesome_feature```, dann:
...
[cli_link]: https://github.com/julienandco/candyman_flutter_cli
[new_creation_option_link]: /add_new_brick#neue-creation-option-in-die-cli-einfügen
